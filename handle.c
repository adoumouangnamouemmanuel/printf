 ...
 */
int handle_write_char(char ch, char buff[], int flgs, int wdth, int prec, int sz)
{
	int i = 0;
	char padding = ' ';
 ...
 * Return: Number of characters printed
 */
int write_number(int is_neg, int index, char buff[], int flgs, int wdth, int prec, int sz)
{
	...
	return (write_num(index, buff, flgs, wdth, prec, length, padding, extra_char));
}
 ...
 * Return: Number of characters printed
 */
int write_num(int index, char buff[], int flgs, int wdth, int prec, int length, char padding, char extra_char)
{
	int i, padd_start = 1;
	...
	if (wdth > length)
	{
		for (i = 1; i < wdth - length + 1; i++)
			buff[i] = padding;
		buff[i] = '\0';
		if (flgs & F_MINUS && padding == ' ')/* Assign extra char to left of buffer */
		{
			if (extra_char)
				buff[--index] = extra_char;
			return (write(1, &buff[index], length) + write(1, &buff[1], i - 1));
		}
		else if (!(flgs & F_MINUS) && padding == ' ')/* Assign extra char to left of buffer */
		{
			if (extra_char)
				buff[--index] = extra_char;
			return (write(1, &buff[1], i - 1) + write(1, &buff[index], length));
		}
		else if (!(flgs & F_MINUS) && padding == '0')/* Assign extra char to left of padding */
		{
			if (extra_char)
 ...
 * Return: Number of characters printed
 */
int write_unsigned(int is_neg, int index, char buff[], int flgs, int wdth, int prec, int sz)
{
	...
	if (wdth > length)
	{
		...
		if (flgs & F_MINUS) /* Assign extra char to left of buffer [buffer > padding] */
		{
 ...
 * Return: Number of characters printed
 */
int write_memory_address(char buff[], int index, int length, int wdth, int flgs, char padding, char extra_char, int padd_start)
{
	int i;
